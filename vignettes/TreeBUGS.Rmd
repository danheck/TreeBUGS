...
### A) Including Covariates

In the latent-trait MPT, predictor variables can be included to account for interindividual variance in MPT parameters. In contrast, in the Beta-MPT, only the correlation of covariates with MPT parameters is sampled (which does not change the estimated model). However, the arguments to include covariates are identical:
...
* `covStructure`: Either a list or path to a text file in which the assignment of MPT parameters to covariates is coded as follows:
    * MPT parameter label(s)  ; label(s)   (separated by a semicolon)
...
### B) Include Transformed Parameters
...
### C) Generate Simulated Data Sets
