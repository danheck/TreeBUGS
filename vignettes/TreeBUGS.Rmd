...
2. Define path to data set with individual frequencies (.csv file: semicolon separated, rows=persons, columns=labeled categories)
...
As default, the data are provided in a semicolon-separated text file (.csv) in the following format:
...
traceplot(fitHierarchicalMPT$mcmc,    # fitted model
...
```{r, eval=FALSE}
# load coda library (convergence diagnostics)
require("coda")

# transform R2JAGS output to mcmc.list:
codaModel <- as.mcmc.list(fitHierarchicalMPT$mcmc$BUGSoutput)

# For instance, auto-correlation plots:
acfplot(codaModel)
```

See \link{R2JAGS} and \link{coda} for further convergence statistics and plots. Note that inferences from from the model are likely not to be valid if the Markov-Chain Monte-Carlo (MCMC) sampler did not reach convergence!
...
In the latent-trait MPT, predictor variables can be included to account for interindividual variance in MPT parameters. In contrast, in the Beta-MPT, only the correlation of covariates with MPT parameters is sampled (which does not influence the parameter estimates). However, the arguments to include covariates are identical:

* `covData`: Either a data frame or the path to a csv data file separated by semicolons `;`
...
* `covStructure`: Either a list or path to a text file in which the assignment of MPT parameters to covariates is coded as follows:
...
fitMPT <- traitMPT(eqnfile = "2htm.txt", 
                   ...
                   covStructure = list("Do Dn ; IQ",  "g ; age extraversion")
                   ...
```
...
* New argument `covType`: Covariates can be set as
    * continuous (`"c"`)
    * fixed effect (`"f"`)
    * random effect (`"r"`)
* `covData` can have columns with character or factor variables (numeric columns can be specified as factors using `covType`)
* By default, character variables in `covData` are included as fixed effects
* The order of `covType` has to match the column order of `covData`
...
fitMPT <- traitMPT(eqnfile = "2htm.txt", 
                   data = "data_ind.csv",   
                   covData = "data_covariates.csv", 
                   covStructure = list("Do Dn ; factor1 factor2",  "g ; age"),
                   covType = c("c", "c", "f", "r")
                   ...
```


Multiple factors can be included, but currently it is not possible to directly include interactions. For an introduction to Bayesian ANOVA, see Rouder et al., 2012.
