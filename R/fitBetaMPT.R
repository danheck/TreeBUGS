...
#' @param dic whether to compute DIC using \code{\link[runjags]{extract.runjags}}, which requires additional sampling. Can also be computed and added after fitting the model by \code{fittedModel$dic <- extract(fittedModel$runjags, "dic")}
...
#' @param autojags if provided (as an empty list or with arguments passed to \link[runjags]{autorun.jags}), JAGS runs repeatedly until the MCMC chains converges . E.g., use \code{list(max.time="30m")} to restrict sampling to 30 minutes (similarly for hours, days, and weeks)
